...
#' Parse Pubmed Citations (from xml)
#'
#' Parses standard fields for PubMed citations, usually obtained
#' by downloading .xml file from PubMed
#'
#' @param .xml path to .xml file; or other inputs manageable by
#' the [xml2](https://github.com/r-lib/xml2) package
#' @param ... other arguments passed on to `xml2::read_xml()`
#' @md
#' @export
parse_pm_xml <- function(.xml, ...) {
    x <- xml2::read_xml(x = .xml, ...)

    assert_that(class(x)[1] == "xml_document") # text for node might be better

}

parse_pm_docsumm <- function(DocumentSummary) {
    assertthat::assert_that(
        xml2::xml_find_all(DocumentSummary, "//Item") == 1
    )

    tibble::tibble(
        authors = xml2::xml_find_all(xml, "//Description"),
        title = xml2::xml_find_all(xml, "//Title"),
        details = xml2::xml_find_all(xml, "//Details"),
        PMID = xml2::xml_find_all(xml, "//Identifier[@name='PMID']"),
        PMCID = xml2::xml_find_all(xml, "//Identifier[@name='PMCID']")
        type = xml2::xml_find_all(xml, "//Type"),

...
#'
#' @export
plot_citedby <- function(
  data_file = "data/citedby/DO_citedby.csv",
  out_dir = "graphics/website",
  color_set = c(
    "Article" = "#4C3E45",
    "Clinical Trial" = "#B9964B",
    "Book" = "#83C85F",
    "Conference" = "#95B1BB",
    "Review" = "#934FBB",
    "Other" = "#C45055",
  ...
  if (length(color_set) == 1) {
    ...
    if (length(color_set) != 7 || !all(names(color_set) %in% color_nm)) {
      rlang::abort(
        "`color_set` must specify a DO_colors color set or 7 named colors"
      )
    }
